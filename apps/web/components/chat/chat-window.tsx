...
import { SendHorizontal, Loader2, Bot, User, FileText, X } from 'lucide-react'
...
// Popup hiện full content của 1 source
function SourceModal({ source, index, onClose }: { source: Source; index: number; onClose: () => void }) {
  return (
    <div
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-3 p-5 border-b border-border shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold truncate">{source.documentTitle}</p>
              <p className="text-xs text-muted-foreground mt-0.5">
                Nguồn {index + 1} &middot;{' '}
                <span className={cn(
                  'font-medium',
                  source.similarity >= 0.8 ? 'text-green-400' : source.similarity >= 0.6 ? 'text-yellow-400' : 'text-muted-foreground'
                )}>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          <p className="text-sm leading-relaxed whitespace-pre-wrap">{source.content}</p>
        </div>

        <div className="px-5 py-3 border-t border-border shrink-0">
          <div className="flex items-center gap-2">
            <div className="h-1.5 w-32 rounded-full bg-secondary overflow-hidden">
              <div className="h-full bg-primary rounded-full" style={{ width: `${source.similarity * 100}%` }} />
            </div>
            <span className="text-xs text-muted-foreground">{(source.similarity * 100).toFixed(1)}% similarity</span>
          </div>
        </div>
      </div>
    </div>
...
// Citations bar dưới message AI
function Citations({ sources }: { sources: Source[] }) {
  const [selected, setSelected] = useState<{ source: Source; index: number } | null>(null)
      ...
      <div className="flex flex-wrap gap-1.5 mt-2">
        ...
        {sources.map((source, i) => (
          ...
          >
            ...
            <Badge variant="secondary" className="text-xs h-4 px-1 font-mono ml-0.5">
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">Ask anything about your documents</p>
            </div>
          </div>
        ) : (
          messages
            ...
            .map((msg) => (
              ...
              >
                ...
                <div className={cn('max-w-[80%]', msg.role === 'USER' ? 'items-end' : 'items-start')}>
                  ...
                  {/* Citations — chỉ hiện với message ASSISTANT có sources */}
                  {msg.role === 'ASSISTANT' &&
      ...
      <div className="px-4 pb-4 pt-2 border-t border-border">
        ...
        <p className="text-xs text-muted-foreground text-center mt-2">Shift+Enter for new line</p>
      </div>
    </div>
  )
}